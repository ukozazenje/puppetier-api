  ...
  axios
    ...
    .then((response) => {
      ...
      $(".ny__yacht").each((index, element) => {
        // Extract yacht data
        const yachtLink = $(element).find(".ny__yacht__img a").attr("href") || ""
        ...
        const priceText = $(element).find(".ny-price").text().replace(/[^\d]/g, "") // Remove non-numeric characters
        const price = priceText ? parseFloat(priceText) : null

        const lengthFeet = $(element).find(".ny-length").text().trim()
        const lengthMeters = $(element).find(".ny-length small").text().replace(/[()M]/g, "").trim()
        const manufacturer = $(element).find(".ny-model").text().trim()
        const year = $(element).find(".ny-year").text().trim()

        const cabinCount = parseInt($(element).find(".ny__yacht__rooms .ny-rooms:first-child").text().replace(/\D/g, "")) || 0
        const sleepCount = $(element).find(".ny__yacht__rooms .ny-rooms:nth-child(2)").text().replace(/\D/g, "") || 0
        const crewCount = $(element).find(".ny__yacht__rooms .ny-rooms:nth-child(3)").text().replace(/\D/g, "") || 0
        ...
        yachts.push({
          ...
          length_display_feet: lengthFeet.split("'")[0] + "'", // Example: "203'"
          length_display_meters: lengthMeters + "M",
          cabin_count: cabinCount,
          sleep_count: parseInt(sleepCount),
          head_count: 0, // Replace with actual head count if available
          crew_head_count: 0, // Replace with actual crew head count if available
          crew_sleep_count: parseInt(crewCount),
          year: year ? parseInt(year) : null,
          refit_year: null, // No refit year available in HTML
          vessel_type: "Power", // Assuming vessel type is "Power"
          vessel_manufacturer: manufacturer,
          vessel_category: "Motor Yachts", // Assuming category
          vessel_subcategory: null, // No subcategory available in HTML
          city: "Antalya", // Assuming city (as per your sample)
          state: null, // No state in the HTML
          country: "TURKEY", // Assuming country (as per your sample)
          thumb_urls: {
            primary: thumbUrl,
            large: thumbUrl,
          },
...
exports.GetYacht = async (req, res, next) => {
  ...
  try {
    ...
    const galleryData = await page.evaluate(() => {
      const pics = Array.from(document.querySelectorAll('.yacht-carousel-img img')).map(img => ({
        src: img.src,
      }));
    ...
    const yachtData = {
      ...
      characteristics: yachtDetailsMainSection.find('.category-filters').next().html(),
      ...
      gallery: {
        pics: $('.yacht-gallery-item').map((i, el) => ({
          title: "",
          imgurl: $(el).css('background-image').replace(/url\("(.+)"\)/, '$1'),
          full: $(el).css('background-image').replace(/url\("(.+)"\)/, '$1'),
          tiny: $(el).css('background-image').replace(/url\("(.+)"\)/, '$1')
        })).get(),
        first: {
          title: "",
          imgurl: $('.yacht-gallery-item').first().css('background-image').replace(/url\("(.+)"\)/, '$1'),
          full: $('.yacht-gallery-item').first().css('background-image').replace(/url\("(.+)"\)/, '$1'),
          tiny: $('.yacht-gallery-item').first().css('background-image').replace(/url\("(.+)"\)/, '$1')
        }
      },
    };
